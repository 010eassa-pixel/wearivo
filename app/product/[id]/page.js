      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '12px' }}>
            {['M', 'L', 'XL', '2XL'].map(size => (
              <button key={size} style={{ padding: '10px 20px', border: '1px solid #cbd5e1', background: '#fff', borderRadius: '6px', cursor: 'pointer' }}>{size}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #cbd5e1', borderRadius: '10px' }}>
            <button onClick={() => setQuantity(q => q + 1)} style={{ padding: '12px 18px', border: 'none', background: 'none', cursor: 'pointer' }}>+</button>
            <span style={{ padding: '0 25px', fontWeight: 'bold' }}>{quantity}</span>
            <button onClick={() => setQuantity(q => q > 1 ? q - 1 : 1)} style={{ padding: '12px 18px', border: 'none', background: 'none', cursor: 'pointer' }}>-</button>
          </div>
          <button style={{ flex: 1, backgroundColor: '#1e3a8a', color: '#fff', padding: '18px', borderRadius: '12px', border: 'none', fontWeight: 'bold', cursor: 'pointer' }}>إضافة إلى السلة</button>
        </div>
        ...
        <div style={{ marginTop: '30px', padding: '20px', backgroundColor: '#f1f5f9', borderRadius: '12px', fontSize: '15px', color: '#475569' }}>
