    ...
    <div style={{ maxWidth: '1200px', margin: '40px auto', padding: '20px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', direction: 'rtl', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', gap: '15px' }}>
         ...
         <div style={{ width: '80px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
            <img src={product.imageUrl} style={{ width: '100%', borderRadius: '8px', border: '2px solid #3b82f6', cursor: 'pointer' }} />
         </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '25px' }}>
        ...
        {/* اختيار المقاس */}
        <div>
          <p style={{ marginBottom: '12px', fontWeight: 'bold', color: '#475569' }}>المقاس:</p>
          <div style={{ display: 'flex', gap: '12px' }}>
            {['M', 'L', 'XL', '2XL'].map(size => (
              <button key={size} style={{ padding: '10px 20px', border: '1px solid #cbd5e1', background: '#fff', borderRadius: '6px', cursor: 'pointer', transition: '0.2s' }}>{size}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '20px', marginTop: '10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #cbd5e1', borderRadius: '10px', backgroundColor: '#f8fafc' }}>
          ...
          <button style={{ flex: 1, backgroundColor: '#1e3a8a', color: '#fff', padding: '18px', borderRadius: '12px', border: 'none', fontWeight: 'bold', cursor: 'pointer', fontSize: '16px', transition: '0.3s' }}>إضافة إلى السلة</button>
        </div>
        ...
        <div style={{ marginTop: '30px', padding: '20px', backgroundColor: '#f1f5f9', borderRadius: '12px', fontSize: '15px', color: '#475569', lineHeight: '1.8' }}>
            <p>• خامة قطن 100% عالية الجودة ومعالجة ضد الانكماش</p>
            <p>• التوصيل خلال 3-5 أيام عمل لجميع المحافظات</p>
            <p>• إمكانية المعاينة والقياس عند الاستلام</p>
        </div>
      </div>
