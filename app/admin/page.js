...
export default function WearivoFinalCorrected() {
  ...
  const handleUploadAndSave = async () => {
    if (!productName || !productPrice || !imageFile) return alert("كمل البيانات");
    setLoading(true);
    try {
      // الرفع لـ Cloudinary (الحساب بتاعك)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'row-reverse', minHeight: '100vh', backgroundColor: '#05070a', color: '#fff', direction: 'rtl' }}>
      ...
      {/* Sidebar - ثابت جهة اليمين (image_81abc1.png) */}
      <aside style={{ 
        width: '280px', backgroundColor: '#0a0d14', borderLeft: '1px solid #1a1f2b', 
        padding: '40px 20px', position: 'fixed', right: 0, height: '100vh' 
      }}>
        ...
        <div onClick={() => signOut(auth)} style={{ position: 'absolute', bottom: '40px', width: '240px', color: '#ff4d4d', cursor: 'pointer', textAlign: 'center', fontWeight: 'bold' }}>
          تسجيل الخروج
        </div>
      </aside>

      {/* Main Content - واخد مسافة عشان السايد بار */}
      <main style={{ flex: 1, marginRight: '280px', padding: '40px' }}>
        
        {/* Header - البحث */}
        <header style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '50px' }}>
          <div style={{ background: '#0a0d14', padding: '12px', borderRadius: '12px', border: '1px solid #1a1f2b', cursor: 'pointer' }}>🔔</div>
          <input 
            type="text" placeholder="ابحث برقم الأوردر..." 
            onChange={(e) => setSearchCode(e.target.value)}
            style={{ width: '400px', padding: '12px 20px', borderRadius: '12px', background: '#0a0d14', border: '1px solid #1a1f2b', color: '#fff', outline: 'none' }} 
          />
        </header>
        ...
        {/* Modal - مطابق لـ image_8194dd.png */}
        {isModalOpen && (
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.9)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
            <div style={{ backgroundColor: '#0a0d14', padding: '40px', borderRadius: '30px', width: '450px', border: '1px solid #1a1f2b' }}>
              <h3 style={{ marginBottom: '25px', textAlign: 'center', fontSize: '22px' }}>تفاصيل المنتج</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                ...
                <select value={category} onChange={(e) => setCategory(e.target.value)} style={{ padding: '15px', borderRadius: '12px', background: '#05070a', border: '1px solid #1a1f2b', color: '#fff', cursor: 'pointer' }}>
              ...
              <div style={{ display: 'flex', gap: '15px', marginTop: '30px' }}>
                ...
                <button onClick={handleUploadAndSave} disabled={loading} style={{ flex: 1, padding: '15px', borderRadius: '12px', background: '#3b82f6', color: '#fff', fontWeight: 'bold' }}>
